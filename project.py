...
class Project(object):
    ...
    def create_gradle_wrapper(self):
        ...
        copy_dir('templates/gradle/wrapper', absolute_path_from(self.abs_path, 'gradle/wrapper'))
    ...
    def create_root_files(self):
        ...
        create_file(absolute_path_from(self.abs_path, 'build.gradle'),
                    read_file_content(os.path.join('templates', 'build.gradle')))
        ...
        create_file(absolute_path_from(self.abs_path, '.gitignore'),
                    read_file_content(os.path.join('templates', 'gitignore')))
        ...
        create_file(absolute_path_from(self.abs_path, 'gradlew'),
                    read_file_content(os.path.join('templates', 'gradle', 'gradlew.sh')))
        create_file(absolute_path_from(self.abs_path, 'gradlew.bat'),
                    read_file_content(os.path.join('templates', 'gradle', 'gradlew.bat')))
    ...
    def create_build_system(self):
        ...
        create_file(absolute_path_from(build_folder, 'dependencies.gradle'),
                    read_file_content(os.path.join('templates', 'buildsystem', 'dependencies.gradle')))
        create_file(absolute_path_from(build_folder, 'version.gradle'),
                    read_file_content(os.path.join('templates', 'buildsystem', 'version.gradle')))
        create_file(absolute_path_from(build_folder, 'project.gradle'),
                    read_file_content(os.path.join('templates', 'buildsystem', 'project.gradle')))
    ...
    def create_quality(self):
        ...
        create_file(absolute_path_from(quality_folder, 'quality.gradle'),
                    read_file_content(os.path.join('templates', 'quality', 'quality.gradle')))
        create_file(absolute_path_from(quality_folder, 'jacoco.gradle'),
                    read_file_content(os.path.join('templates', 'quality', 'jacoco.gradle')))
        create_file(absolute_path_from(quality_folder, 'LICENSE'),
                    read_file_content(os.path.join('templates', 'quality', 'LICENSE')))
        copy_dir(os.path.join('templates', 'quality', 'checkstyle'), absolute_path_from(quality_folder, 'checkstyle'))
        copy_dir(os.path.join('templates', 'quality', 'findbugs'), absolute_path_from(quality_folder, 'findbugs'))
        copy_dir(os.path.join('templates', 'quality', 'lint'), absolute_path_from(quality_folder, 'lint'))
        copy_dir(os.path.join('templates', 'quality', 'pmd'), absolute_path_from(quality_folder, 'pmd'))
    ...
    def create_module(self):
        ...
        create_file(absolute_path_from(app_folder, 'build.gradle'),
                    read_file_content(os.path.join('templates', 'module', 'build.gradle')) % internal_pkg)
        create_file(absolute_path_from(android_test_folder, 'ExampleInstrumentedTest.java'),
                    read_file_content(os.path.join('templates', 'module', 'ExampleInstrumentedTest.java'))
                    % (internal_pkg, internal_pkg))
        create_file(absolute_path_from(test_folder, 'ExampleUnitTest.java'),
                    read_file_content(os.path.join('templates', 'module', 'ExampleUnitTest.java')) % internal_pkg)
        create_file(absolute_path_from(main_folder, "{}.java".format(application_name)),
                    read_file_content(os.path.join('templates', 'module', 'BaseApplication.java')) % (
                        internal_pkg, application_name))
        create_file(absolute_path_from(debug_folder, 'DefaultApplication.java'),
                    read_file_content(os.path.join('templates', 'module', 'DebugApplication.java')) % (
                        internal_pkg, application_name))
        create_file(absolute_path_from(release_folder, 'DefaultApplication.java'),
                    read_file_content(os.path.join('templates', 'module', 'ReleaseApplication.java')) % (
                        internal_pkg, application_name))
        create_file(absolute_path_from(release_folder, 'CrashLibrary.java'),
                    read_file_content(os.path.join('templates', 'module', 'CrashLibrary.java')) % internal_pkg)
        create_file(absolute_path_from(base_folder, 'AndroidManifest.xml'),
                    read_file_content(os.path.join('templates', 'module', 'AndroidManifest.xml')) % (
                        internal_pkg, activity_name))
        create_file(absolute_path_from(main_folder, "{}.java".format(activity_name)),
                    read_file_content(os.path.join('templates', 'module', 'MainActivity.java')) % (
                        internal_pkg, activity_name))
        create_file(absolute_path_from(layout_folder, 'activity_main.xml'),
                    read_file_content(os.path.join('templates', 'module', 'activity_main.xml')) %
                    (internal_pkg, activity_name))
        create_file(absolute_path_from(values_folder, 'strings.xml'),
                    read_file_content(os.path.join('templates', 'module', 'strings.xml')) % activity_name)
        create_file(absolute_path_from(values_folder, 'styles.xml'),
                    read_file_content(os.path.join('templates', 'module', 'styles.xml')))
        create_file(absolute_path_from(values_folder, 'colors.xml'),
                    read_file_content(os.path.join('templates', 'module', 'colors.xml')))
